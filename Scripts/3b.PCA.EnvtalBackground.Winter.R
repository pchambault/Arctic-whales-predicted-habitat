############################################################################
############################################################################
###### PCA on bowheads, narwhals, belugas both sides of Greenland in Winter
...
# Nar refers to Narwhal, Bw to Bowhead
...
model  = models[[1]]   # run separately for each model
...
# "1a.PCA.Prepare.Data.Summer_...R"
  ...
  for (f in 1:length(files)) { 
    setwd(paste0("/Users/philippinechambault/Documents/POST-DOC/2021/PAPER3",
                 "/RDATA/3.ACP/6variables/",model,"/",j,"/Env_raster/Winter/"))
    ras  = readRDS(files[[f]])
    d = gsub(".*[_]([^.]+)[.].*", "\\1", files[f])
    
    
    # extract coordinates of raster brick with original resolution
    ...
    saveRDS(xy,file=paste0("./RDATA/3.ACP/6variables/",model,"/",j,
                           "/PCA/Winter/xy_rasterBrick_Winter_",model,"_",
                           j,"_",d,".rds"))
    gc()
...
#-----------------------------
  for (i in sp) { 
    ...
    for (f in 1:length(files)) { 
      setwd(paste0("/Users/philippinechambault/Documents/POST-DOC/2021/PAPER3",
                   "/RDATA/3.ACP/6variables/",model,"/",i,"_",j,"/Env_raster/Winter/"))
      ras  = readRDS(files[[f]])
      d = gsub(".*[_]([^.]+)[.].*", "\\1", files[f])
      ...
      saveRDS(xy,file=paste0("./RDATA/3.ACP/6variables/",model,"/",i,"_",j,
                             "/PCA/Winter/xy_rasterBrick_Winter_",model,"_",i,"_",
                             j,"_",d,".rds"))
      gc()
      ...
      writeRaster(ordi2,filename=paste0("./RDATA/3.ACP/6variables/",model,"/",i,"_",j,
                                        "/PCA/Winter/Ordi_Winter_",model,i,"_",
                                        j,"_",d,".tif"), 
                  format="GTiff",overwrite=TRUE,
                  options=c("INTERLEAVE=BAND","COMPRESS=LZW"))
      gc()
      ...
      saveRDS(ordi_pix,file=paste0("./RDATA/3.ACP/6variables/",model,"/",i,"_",j,
                                   "/PCA/Winter/OrdiPix_Winter_", model,"_",i,"_",
                                   j,"_",d,".rds"))
      saveRDS(ENVspace,file=paste0("./RDATA/3.ACP/6variables/",model,"/",i,"_",j,
                                   "/PCA/Winter/ENVspace_Winter_",model,"_",i,"_",
                                   j,"_",d, ".rds"))
      gc()
    }
  }
...
saveRDS(bwE, "./RDATA/4.Pseudo-abs/Winter/Bw_East_withoutNA_0.25deg.rds")
...
saveRDS(west[west$species=="Bw",], 
        "./RDATA/4.Pseudo-abs/Winter/Bw_West_withoutNA_0.25deg.rds")
saveRDS(west[west$species=="Nar",], 
        "./RDATA/4.Pseudo-abs/Winter/Nar_West_withoutNA_0.25deg.rds")
