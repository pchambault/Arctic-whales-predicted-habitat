...
perf <- readRDS(paste0("/Users/philippinechambault/Documents/POST-DOC/2021/PAPER3",
                       "/RDATA/5.CARET/Performance/PerfMOD_Summer_7algo_6var_3sp_",model,"_.rds"))
perf
pivot = perf %>%
  select(c(accur, sensi, speci,model,run,species,side)) %>%
...
best = pivot %>% 
  ...
  # filter(metric=="accur" | metric=="sensi" | metric=="speci") %>%
...
system.time({ # 207 sec (4 min)
  ...
  foreach(n=1:10) %dopar% { 
    ...
    data <- readRDS(paste0("/Users/philippinechambault/Documents/POST-DOC/2021/PAPER3",
                           "/RDATA/5.CARET/",i,"/Summary_models/Summer/",model,"/split/",
                           "train_Summer_6var_",model,"_",i,"_run",n,".rds"))
    ...
    ffs <- readRDS(paste0("/Users/philippinechambault/Documents/POST-DOC/2021/PAPER3",
                          "/RDATA/5.CARET/",i,"/Summary_models/Summer/",model,
    ...
    #-----------------------------------------
    # run best algo according to validation
    #-----------------------------------------
    fit <- train(data[,predictors], data$pres2, 
                 preProc  = c("center", "scale", "nzv"),
                 method   = algo, 
                 tuneGrid = grid,
                 trControl= ctrl) # blackboost:0.93, nnet:0.89, nnet tuned:0.92
    
    saveRDS(fit, file=paste0("./RDATA/5.CARET/",i,
                             "/Summary_models/Summer/",model,
                             "/Best_Model/BestModel_ffs_Summer_6var_",model,"_",
...
##########################################
#  Bowheads West: blackboost (BRT)
##########################################
i    = "Bw_West"
# algo = "mlpWeightDecayML"
algo = "blackboost"
...
system.time({ # 60 sec
  ...
  foreach(n=1:10) %dopar% { 
    ...
    saveRDS(fit, file=paste0("./RDATA/5.CARET/",i,
                             "/Summary_models/Summer/",model,
                             "/Best_Model/BestModel_Summer_6var_",model,"_",
                             i,"_",m$method,"_run",n,".rds"))
  }    # loop over each run
  gc()
  stopCluster(cl)
